...
# ============================================================
# SUPABASE - DAILY REPORT PERSISTENCE
# ============================================================

SUPABASE_PHOTO_BUCKET = "daily-report-photos"
...
def _json_safe(value):
    """Convert Streamlit/Python values to JSON-safe values for JSONB."""
...
def _parse_date_value(value, fallback=None):
...
def _parse_time_value(value, fallback=None):
    if isinstance(value, time):
        return value
    if isinstance(value, str) and value:
        try:
...
def _restore_daily_report_payload(payload):
    """Restore dates/times after reading a JSONB Daily Report payload."""
    record = dict(payload or {})

    for field in ["Created", "Last Updated", "Date"]:
        if field in record:
            restored = _parse_date_value(record.get(field))
            if restored is not None:
                record[field] = restored

    for field in ["Working Start", "Working Finish"]:
        if field in record:
            restored = _parse_time_value(record.get(field))
            if restored is not None:
                record[field] = restored
...
def ensure_daily_photo_bucket():
    """Ensure that the private bucket used by Daily Report photos exists."""
    supabase = get_supabase_client()

    try:
        supabase.storage.get_bucket(SUPABASE_PHOTO_BUCKET)
        return
    except Exception:
        pass

    # Current Supabase Python clients expose create_bucket with options.
    # A fallback call is included for compatibility with older releases.
    try:
...
def load_daily_reports_from_supabase():
    """Load all persisted Daily Report data into Streamlit session_state."""
    supabase = get_supabase_client()
    ...
    for row in (report_response.data or []):
        ...
        if not report_key:
            continue
        loaded_reports[report_key] = _restore_daily_report_payload(
            row.get("payload") or {}
        )

    st.session_state.daily_reports = loaded_reports
    ...
    for table_name, state_name in child_tables:
        response = (
            supabase
            .table(table_name)
            .select("report_key,payload")
        ...
        for row in (response.data or []):
            ...
            payload["Daily Report Key"] = row.get("report_key")
            collection.append(payload)

        st.session_state[state_name] = collection

    # Photos are stored in Supabase Storage and their metadata lives in daily_photos.
    photo_response = (
        supabase
        .table("daily_photos")
        .select("id,report_key,file_name,location,description,storage_path")
        .execute()
    )
    ...
    for row in (photo_response.data or []):
        photo = {
            "Daily Report Key": row.get("report_key"),
            "File Name": row.get("file_name") or "Site Photo",
            "Location": row.get("location") or "",
            "Description": row.get("description") or "",
            "Storage Path": row.get("storage_path") or "",
        }

        storage_path = photo["Storage Path"]
        if storage_path:
            try:
                photo["Bytes"] = (
                    supabase.storage
...
def save_daily_report_to_supabase(report_key, report_record):
    """Persist one Daily Report and all its child records to Supabase."""
    supabase = get_supabase_client()
    warnings = []

    # Parent report must exist before child records because of foreign keys.
    supabase.table("daily_reports").upsert(
        {
            "report_key": report_key,
            "payload": _json_safe(report_record),
    ...
    child_tables = [
        ("daily_manpower", st.session_state.daily_manpower),
        ("daily_equipment", st.session_state.daily_equipment),
        ("daily_activities", st.session_state.daily_activities),
        ("daily_deliveries", st.session_state.daily_deliveries),
        ("daily_inspections", st.session_state.daily_inspections),
        ("daily_issues", st.session_state.daily_issues),
    ]

    # Replace the child records for this report with the current app state.
    for table_name, collection in child_tables:
        (
            supabase
            .table(table_name)
            .delete()
        ...
        rows = []
        for item in daily_items_for_key(collection, report_key):
            payload = dict(item)
            payload.pop("Daily Report Key", None)
            rows.append(
                {
                    "report_key": report_key,
    ...
    # Persist photos. Existing files for this report are replaced so that
    # deletions/edits performed in Streamlit are reflected in the database.
    try:
        ensure_daily_photo_bucket()
        ...
        old_paths = [
            ...
            for row in (old_photo_response.data or [])
        ...
        if old_paths:
            try:
                ...
                warnings.append(f"Old photo cleanup: {exc}")
        ...
        current_photos = daily_items_for_key(
        ...
        for photo in current_photos:
            image_bytes = photo.get("Bytes") or b""
            if not image_bytes:
                continue

            file_name = photo.get("File Name") or "site_photo.jpg"
            safe_file_name = _safe_storage_name(file_name)
            report_folder = _safe_storage_name(report_key)
            storage_path = (
                f"{report_folder}/"
            ...
            photo["Storage Path"] = storage_path
        ...
        if photo_rows:
            supabase.table("daily_photos").insert(photo_rows).execute()

    except Exception as exc:
        warnings.append(f"Photos were not persisted: {exc}")

    return warnings


def refresh_daily_reports_from_supabase():
    """Force a fresh database read after saving."""
    load_daily_reports_from_supabase()
    st.session_state.supabase_daily_loaded = True


# Load persisted Daily Reports once for each browser session.
if not st.session_state.get("supabase_daily_loaded", False):
    try:
        load_daily_reports_from_supabase()
        st.session_state.supabase_daily_loaded = True
        st.session_state.supabase_daily_error = ""
    except Exception as exc:
        # Keep the app usable even if the internet/database is temporarily unavailable.
        st.session_state.supabase_daily_error = str(exc)
        st.session_state.supabase_daily_loaded = False
...
# ============================================================
    ...
    if st.session_state.get("supabase_daily_loaded", False):
        st.success(
            "☁️ Database connected — Daily Reports are saved permanently in Supabase.",
            icon=None,
        )
    elif st.session_state.get("supabase_daily_error"):
        st.warning(
            "Database connection is currently unavailable. "
            "Daily Report permanent storage is not active. "
            f"{st.session_state.supabase_daily_error}"
        )
    ...
    if (
        ...
        if (
            ...
            if duplicate:
                ...
                # Persist the Daily Report and all child records in Supabase.
                try:
                    supabase_warnings = save_daily_report_to_supabase(
                        final_daily_key,
                        report_record,
                    )
                    ...
                    for warning_text in supabase_warnings:
                        st.warning(
                            "Daily Report saved, but "
                            f"{warning_text}"
                        )

                except Exception as exc:
                    st.error(
                        "The Daily Report was kept in this browser session, "
                        "but could not be saved permanently to Supabase. "
                        f"Error: {exc}"
                    )
                    st.stop()
